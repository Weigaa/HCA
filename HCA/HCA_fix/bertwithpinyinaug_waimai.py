...
#classnumber
classnumber = 2
dataset_path = "waimai_10k_shuffle.csv"
maxlen = 127
testsamples = 1198
validationsamples = 1198
...
def train_and_eval(model, train_loader, valid_loader, optimizer, scheduler, device, epoch):
    ...
    for i in range(epoch):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), "best_bert_model_waimai.pth")
        print("current acc is {:.4f}, best acc is {:.4f}".format(acc, best_acc))
        print("time costed = {}s \n".format(round(time.time() - start, 5)))


train_and_eval(model, train_loader, valid_loader, optimizer, scheduler, DEVICE, EPOCHS)
model.load_state_dict(torch.load("best_bert_model_waimai.pth"))
