...
#classnumber
classnumber = 10
dataset_path = "online_shopping_10_cats_shuffle.csv"
...
def pinyinaug(inputtext = None):
    ...
    if choice < alpha:
        ...
        if len(dagresult) == 0:
            ...
            return newtitlelist
        for i in dagresult:
            newtitlelist.append(''.join(i.path))
        return newtitlelist
...
with open(dataset_path, encoding='utf-8') as f:
    for i, line in tqdm(enumerate(f)):
        ...
        # 根据参数会短则补齐，长则切断
        if i < trainsamples/num_per_sample:
        ...
        if i >= trainsamples/num_per_sample and i < trainsamples/num_per_sample+validationsamples+testsamples:
            encode_dict = tokenizer.encode_plus(text=title, max_length=maxlen,
                                                padding='max_length', truncation=True)
            input_ids.append(encode_dict['input_ids'])
            input_types.append(encode_dict['token_type_ids'])
        ...
        if i == trainsamples/num_per_sample+testsamples+validationsamples:
            break
...
def train_and_eval(model, train_loader, valid_loader, optimizer, scheduler, device, epoch):
    ...
    for i in range(epoch):
        ...
        if acc > best_acc:
            best_acc = acc
            torch.save(model.state_dict(), "best_bert_model_shopping.pth")
        print("current acc is {:.4f}, best acc is {:.4f}".format(acc, best_acc))
        print("time costed = {}s \n".format(round(time.time() - start, 5)))


train_and_eval(model, train_loader, valid_loader, optimizer, scheduler, DEVICE, EPOCHS)
model.load_state_dict(torch.load("best_bert_model_shopping.pth"))
